...
import { CourseProgress , TopRanking , AverageTime , ActiveUser} from '../interfaces/dashboard';
...
export const fetchUserActive = async ( enterpriseId?: number): Promise<ActiveUser[]> => {
  const response = await axios.get(`${API_DASHBOARD}/active-students/${enterpriseId}`);
  return response.data;
};
