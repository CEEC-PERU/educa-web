...
export const getUserEnterprise = async (userToken: string, userId: number): Promise<UserEnterprise | null> => {
  try {
    const config = {
      headers: {
    ...
    const response = await axios.get<UserEnterprise>(`${API_GET_EMPRESA_BY_USER}/${userId}`, config);
    if (response.data) {
