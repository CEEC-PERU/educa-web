  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_GET_NOTAS_EXCEL}/calidad/${userInfo.enterprise_id}/${courseIdNumber}`,
        {
      ...
      <div className="flex flex-1 pt-16">
        ...
        >
          ...
          <div className="flex items-center space-x-4 mb-10">
            ...
            <button
              className="text-white bg-blue-600 px-8 rounded-lg p-2"
                ...
                <>
                  <table className="min-w-full bg-white border border-blue-300 shadow-md">
                    <thead className="bg-blue-500 text-white">
                      <tr>
                        <th
                          rowSpan={2}
                          className="py-2 px-4 border-b border-l-4 border-blue-300"
                        >
                      ...
                      <tr>
                        <th className="py-2 px-4 border-b border-l-4 border-blue-300">
                          Nota 1
                        </th>
                        <th className="py-2 px-4 border-b border-l-4 border-blue-300">
                          Nota 2
                      ...
                      {currentCourseData.map((user: any, userIndex: number) => (
                        ...
                        >
                          ...
                          {currentCourseData[0]?.ModuleResults?.map(
                            (module: any, moduleIndex: number) => {
                              ...
                              if (moduleResult) {
                                ...
                                return (
                                  <td
                                    key={`module-highest-${userIndex}-${moduleIndex}`}
                                    className="py-2 px-4 border-b border-l-4 border-blue-300 text-black"
                                  >
                                    {highestScore || '-'}
                                  </td>
                                );
                                ...
                                return (
                                  <td
                                    key={`empty-module-${userIndex}-${moduleIndex}`}
                                    className="py-2 px-4 border-b border-l-4 border-blue-300 text-black"
                                  >
                                    -
                                  </td>
                          ...
                          {['Nota 1', 'Nota 2'].map((label, examIndex) => {
                            const result =
                              user.CourseResults?.[examIndex]?.puntaje || '-';
                            return (
                              <td
                                key={`exam-${userIndex}-${examIndex}`}
                                className="py-2 px-4 border-b border-l-4 border-blue-300 text-black"
                              >
                                {result}
                              </td>
                  ...
                  <div className="mt-10">
                    <h3 className="text-2xl font-bold mb-4">
                      Reporte de Aprobados y Desaprobados
                    </h3>
                    <Chart
                      type="bar"
                      options={{
                  ...
                  <div className="mt-10">
                    <h3 className="text-2xl font-bold mb-4">
                      Participación Diaria por Estudiante
                    </h3>
                    <Chart
                      type="line"
                      options={{
