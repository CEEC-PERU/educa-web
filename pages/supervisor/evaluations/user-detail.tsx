    ...
    <>
      ...
      {(!user_id || !evaluation_sche_id) && (
        <div className="bg-amber-100 border border-amber-400 text-amber-700 px-4 py-3 rounded">
          <strong>Información requerida:</strong> Se necesitan los parámetros
          user_id y evaluation_sche_id en la URL.
        </div>
      )}
                ...
                {selectedAttempt.evaluation.questions
                  ...
                  .map((question, questionIndex) => {
                    ...
                    const userSelectedOption = question.options.find(
                      (option) =>
                        option.option_sche_id === userAnswer?.option_sche_id,
                    );
                        ...
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <div className="text-right">
                            <div className="text-sm text-gray-600">
                              Puntos obtenidos: {userAnswer?.points_earned || 0}
                              /{question.points}
                            </div>
                            {userAnswer && (
                              <div className="text-xs text-gray-500 mt-1">
                                Tiempo:{" "}
                                {Math.round(userAnswer.time_spent_seconds / 60)}{" "}
                                min
                              </div>
                            )}
                          </div>
