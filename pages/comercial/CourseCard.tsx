...
const CourseCard: React.FC<CourseCardProps> = ({ course }) => {
  ...
  const getStatusButton = (progress: number) => {
    if (progress === 100) {
      return (
        <div className="bg-green-500 text-white px-3 py-1 rounded-full mt-4 text-center">
          Finalizado
        </div>
      );
    } else if (progress > 0) {
      return (
        <div className="bg-yellow-500 text-white px-3 py-1 rounded-full mt-4 text-center">
          En progreso
        </div>
      );
    }
      ...
      <div className="flex flex-col md:flex-row">
        <div className="mr-4">
          ...
          <div className="flex items-center mt-4">
            ...
            <span className="text-gray-600 ml-3 pr-10 text-center">
        ...
        <div className="flex-grow">
          <div className="flex justify-between items-center">
            ...
            <div className="flex justify-center items-center w-1/3">
              <CircularBar
                percentage={course.completedPercentage}
                label="Finalizado"
              />
            </div>
            <div className="flex justify-center items-center w-1/3">
              <CircularBar
                percentage={course.approvedPercentage}
                label="Aprobado"
              />
            </div>
          </div>
