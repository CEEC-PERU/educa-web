...
const StudentIndex: React.FC = () => {
  ...
  const startDrawing = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (!canvas) return; // Verificación adicional
    const ctx = canvas.getContext('2d');
    if (!ctx) return; // Asegurarse de que el contexto existe
    ...
    // Usar una función para mantener el contexto
    canvas.addEventListener('mousemove', handleDraw);
    canvas.addEventListener('touchmove', handleDraw);

    // Remover eventos al finalizar
    const stopDrawing = () => {
    ...
    <div>
      ...
      >
        <div className="bg-white rounded-lg p-6 shadow-lg relative">
          ...
          <button onClick={saveSignature} className="bg-green-500 text-white p-2 rounded">Guardar Firma</button>
