        ...
        {userNota.ModuleResults?.length > 0 && (
          <div className="max-h-80 p-4 bg-gray-50 rounded-lg shadow-inner ">
            ...
            {userNota.ModuleResults.map((moduleResult: any) => (
              <div key={moduleResult.module_id} className="bg-white p-4 rounded-lg shadow-md mb-4">
        ...
        {userNota.CourseResults?.length > 0 && (
          <div className="max-h-40 p-4 bg-gray-50 rounded-lg shadow-inner ">
            ...
            {userNota.CourseResults.map((courseResult: any) => (
              <div key={courseResult.course_result_id} className="bg-white p-4 rounded-lg shadow-md mb-4">
  ...
  return (
    <ProtectedRoute>
      <div>
        ...
        <div className="min-h-screen flex flex-col items-center bg-gradient-to-r from-brand-100 via-brand-200 to-brand-300 p-4 pt-12">
            ...
            {courseStudent.map((courseStudentItem) => (
              <div
                ...
                className={`bg-white shadow-lg rounded-lg transform transition-all duration-300 ${
                  selectedCourseId === courseStudentItem.Course.course_id ? 'scale-105' : ''
                }`}
              >
                <h3 className="text-xl font-bold text-gray-900 mb-4 text-center">{courseStudentItem.Course.name}</h3>
                <img
                ...
                <button
                  ...
                  onClick={() => handleToggleNotas(courseStudentItem.Course.course_id)}
                >
                  {selectedCourseId === courseStudentItem.Course.course_id ? 'Ocultar Notas' : 'Ver Notas'}
                </button>
