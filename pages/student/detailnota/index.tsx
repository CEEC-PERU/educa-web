...
import { Course , CourseResult , ModuleResultDetails , ModuleResult } from '../../../interfaces/Nota';
...
const DetailNota: React.FC = () => {
  ...
  const renderNotas = () => {
    if (!courseNota || courseNota.length === 0) return <p>No hay notas disponibles</p>;
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        ...
        {userNota.ModuleResults?.map((moduleResult: ModuleResult) => (
          <div key={moduleResult.module_id} className="p-4 bg-white shadow-lg rounded-lg">
            <h3 className="text-lg font-bold text-brand-500">Modulo: {moduleResult.module_name}</h3>
            {moduleResult.results.map((result: ModuleResultDetails, index: number) => (
              <div key={index}>
                <p className="text-sm text-gray-500">Puntaje: {result.puntaje}</p>
                <p className="text-sm text-gray-400">Fecha: {new Date(result.created_at).toLocaleDateString()}</p>
              </div>
            ))}
          </div>
        ))}
         ...
         {userNota.CourseResults?.map((courseResult: CourseResult) => (
          <div key={courseResult.course_result_id} className="p-4 bg-white shadow-lg rounded-lg">
            <h3 className="text-lg font-bold text-brand-500">Nota Final :{courseResult.Course.name}</h3>
            <p className="text-sm text-gray-500">Puntaje: {courseResult.puntaje}</p>
            <p className="text-sm text-gray-400">Fecha: {new Date(courseResult.created_at).toLocaleDateString()}</p>
          </div>
        ))}
      </div>
    );
