...
const StepThree: React.FC<StepThreeProps> = ({
  ...
  const [correctOptionErrors, setCorrectOptionErrors] = useState<{
    [key: number]: boolean;
  }>({});
  ...
  const addOption = (questionIndex: number) => {
    ...
    questionOptions.push({
      option_text: '',
      is_correct: false,
      question_id: questionsData[questionIndex].evaluation_id,
    });
    newOptions[questionIndex] = questionOptions;
    setLocalOptionsData(newOptions);
  };
  ...
  const handleBlur = (
    questionId: number,
    optionIndex: number,
    field: keyof Omit<Option, 'option_id'>
  ) => {
    ...
    setErrors((prev) => ({
      ...
      [questionId]: prev[questionId]
        ? [
            ...prev[questionId],
            !localOptionsData[questionId][optionIndex][field],
          ]
        : [!localOptionsData[questionId][optionIndex][field]],
    }));
  };
  ...
  const handleNext = () => {
    ...
    Object.keys(localOptionsData).forEach((questionId) => {
      newErrors[Number(questionId)] = localOptionsData[Number(questionId)].map(
      ...
      if (newErrors[Number(questionId)].includes(true)) {
        ...
        setTouchedFields((prev) => ({
          ...prev,
          [Number(questionId)]: localOptionsData[Number(questionId)].map(
            () => true
          ),
        }));
      }

      const hasCorrectOption = localOptionsData[Number(questionId)].some(
    ...
    <WizardStepContainer>
      {questionsData.map((question, questionIndex) => (
        <div key={questionIndex} className="mb-4">
          ...
          {(localOptionsData[questionIndex] || []).map(
            (option, optionIndex) => (
              <div key={optionIndex} className="mb-4">
                ...
                <input
                  ...
                  onBlur={() =>
                    handleBlur(questionIndex, optionIndex, 'option_text')
                  }
                  className={`shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline ${
                    touchedFields[questionIndex]?.[optionIndex] &&
                    errors[questionIndex]?.[optionIndex]
                      ? 'border-red-500'
                      : ''
                  }`}
                />
                ...
                {correctOptionErrors[questionIndex] &&
                  optionIndex ===
                    localOptionsData[questionIndex].length - 1 && (
          ...
          <button
            ...
            className="py-2 px-4 bg-green-600 text-white rounded-md mr-4 flex items-center mt-4"
          >
      ...
      <div className="flex justify-between mt-4">
        <button
          onClick={prevStep}
          className="py-2 px-4 bg-gray-600 text-white rounded-md mr-4 flex items-center"
        >
