...
interface ActionButtonsProps {
  ...
  isEditing?: boolean;
  customSize?: boolean; // Añadir esta prop para controlar el tamaño personalizado
}

const ActionButtons: React.FC<ActionButtonsProps> = ({ onEdit, onCancel, onDelete, onSave, isEditing, onAbort, customSize }) => {
  return (
    <div className={`w-full p-4 bg-gradient-to-br from-gray-200 to-gray-400 rounded-lg shadow-md ${customSize ? 'w-[400px]' : ''}`}>
      <div className={`flex flex-col space-y-2 ${customSize ? 'md:w-96 w-full' : ''}`}>
        {!isEditing && onEdit && (
        ...
        {!isEditing && onAbort && (
          <button onClick={onAbort} className="w-full relative inline-flex items-center justify-center p-0.5 mb-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-gray-400 to-gray-600 group-hover:from-gray-400 group-hover:to-gray-600 dark:text-white dark:hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-gray-300 dark:focus:ring-gray-700">
            <span className="w-full relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0">
              Volver
            </span>
          </button>
